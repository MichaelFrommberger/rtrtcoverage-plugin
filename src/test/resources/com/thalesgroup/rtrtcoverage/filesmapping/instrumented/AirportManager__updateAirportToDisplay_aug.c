...
_ATC_DECLARE(1,0x36A7D4AUL,0x52152A27UL,2,_ATC_TAB_PROC(1),0,0,5,_ATC_TAB_BLOC(1),0,0)

#line 1 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToDisplay.c"
...
#line 17 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToDisplay.c"
...
#line 23 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToDisplay.c"
...
void AirportManager__updateAirportToDisplay(AirportManager *pThis)
{_ATC_LINK(1);{_ATC_PROC(1,1);{_ATC_BLOC(1,4);{
     
	AirportManagerComponent *lAirportManagerComponent =  ((T_ptr)0) ;
	PointerArray *lAirportToDisplayModificationEvents =  ((T_ptr)0) ;
	AirportToDisplayModificationEvent *lAirportToDisplayModificationEvent =  ((T_ptr)0) ;
	T_uint32 lCount = 0;
	T_uint32 lI = 0;

	lAirportManagerComponent = CAirportManagerComponent->getInstance();
	 
	lAirportToDisplayModificationEvents =
			(PointerArray*)(lAirportManagerComponent->getConnectedFacet(lAirportManagerComponent,
										E_AIRPORT_MANAGER_AIRPORT_TO_DISPLAY_MODIFICATION_EVENT_RECEPTACLE_INDEX));
	lCount = lAirportToDisplayModificationEvents->getCount(lAirportToDisplayModificationEvents);

	{_ATC_LOOP_bef(1,1);for( lI = 0; lI < lCount; lI++ )
	{_ATC_LOOP_in(1,1);{_ATC_BLOC(1,0);{
		lAirportToDisplayModificationEvent = (AirportToDisplayModificationEvent*)(lAirportToDisplayModificationEvents->get(lAirportToDisplayModificationEvents, lI));
		lAirportToDisplayModificationEvent->airportToDisplayChanged(lAirportToDisplayModificationEvent, pThis->_requestedAirportForDisplay);
	}}}}
