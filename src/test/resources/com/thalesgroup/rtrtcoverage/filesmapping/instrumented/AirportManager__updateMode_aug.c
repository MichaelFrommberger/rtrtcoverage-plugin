...
_ATC_DECLARE_BLOC(1,22)
_ATC_DECLARE(1,0x48362100UL,0x17207D11UL,2,_ATC_TAB_PROC(1),0,0,22,_ATC_TAB_BLOC(1),0,0)

#line 1 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
...
#line 17 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
...
#line 23 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
#line 24 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
#line 25 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
#line 26 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
...
#line 27 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
#line 28 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
...
#line 29 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateMode.c"
...
void AirportManager__updateMode(AirportManager *pThis) 
{_ATC_LINK(1);{_ATC_PROC(1,1);{_ATC_BLOC(1,21);{
	...
	MapObjectServices *lPlanAirportObject =  ((T_ptr)0) ;
	T_char *lICAOAirport =  ((T_ptr)0) ;
	PointerArray *lAirportRequestActivationEvents =  ((T_ptr)0) ;
	T_boolean lAirportRequestEnabled =  1 ;
	T_uint32 lCount = 0;
	T_uint32 lI = 0;
	AirportRequestActivationEvent *lAirportRequestActivationEvent =  ((T_ptr)0) ;
	ModeManagerServices *lModeManagerServices =  ((T_ptr)0) ;

	lAirportManagerComponent = CAirportManagerComponent->getInstance();
	lModeManagerServices =
				(ModeManagerServices*)(lAirportManagerComponent->getConnectedFacet(lAirportManagerComponent,
													E_AIRPORT_MANAGER_MODE_MANAGER_SERVICES_RECEPTACLE_INDEX));
	pThis->_mode = lModeManagerServices->getMode(lModeManagerServices);

	 

	if ((pThis->_mode == E_ARC_MODE) || (pThis->_mode == E_NAV_MODE))
	{_ATC_BLOC(1,2);{
		if (pThis->_requestedAirportForDisplay != pThis->_airportToBeDisplayedInArcNav)
		{_ATC_BLOC(1,0);{
			pThis->_requestedAirportForDisplay = pThis->_airportToBeDisplayedInArcNav;
			pThis->_updateAirportToDisplay(pThis);
		}}
		else
		{_ATC_BLOC(1,1);{
	...
	if (pThis->_mode != pThis->_previousMode)
	{_ATC_BLOC(1,19);{
		pThis->_previousMode = pThis->_mode;
		...
		switch (pThis->_mode)
		{
			case E_PLAN_MODE:{_ATC_BLOC(1,4);
			{
				if (pThis->_firstEntryPlanModeAfterTransition ==  1 )
				{_ATC_BLOC(1,5);{
					pThis->_firstEntryPlanModeAfterTransition =  0 ;
					pThis->_firstDisplayModePlanAfterTransition =  1 ;
					pThis->_updateAirportToBeDisplayedInPlan(pThis);
				}}
				else
				{_ATC_BLOC(1,8);{
					if (pThis->_firstDisplayModePlanAfterTransition ==  1 )
					{_ATC_BLOC(1,6);{
						pThis->_firstDisplayModePlanAfterTransition =  0 ;
						pThis->_updateAirportToBeDisplayedInPlan(pThis);
					}}
					else
					{_ATC_BLOC(1,7);{
				...
				if (pThis->_requestedAirportForDisplay != pThis->_airportToBeDisplayedInPlan)
				{_ATC_BLOC(1,9);{
					pThis->_requestedAirportForDisplay = pThis->_airportToBeDisplayedInPlan;
				...
				if (pThis->_requestedAirportForDisplay !=  -1 )
				{_ATC_BLOC(1,11);{
					 
					lPlanAirportObject	= lDatabaseServices->getAirportInstance(lDatabaseServices, pThis->_requestedAirportForDisplay);
					lICAOAirport		= lMapObjectServices->getCharData(lMapObjectServices, lPlanAirportObject,  11 );

					 
					CStringUtilities->ncopy(&(pThis->_airportNameToBeDisplayedInPlan[0]), lICAOAirport,  5 );
				}}
				else
				{_ATC_BLOC(1,12);{
					CStringUtilities->ncopy(&(pThis->_airportNameToBeDisplayedInPlan[0]), "",  5 );
				}}

				pThis->_lastDisplayedAirportInPlan = pThis->_airportToBeDisplayedInPlan;
				lAirportRequestEnabled =  1 ;
				pThis->_memorizedAirportInArcNav =  -1 ;
			}}
			break;

			case E_ARC_MODE:
			case E_NAV_MODE:{_ATC_BLOC(1,13);
			{
				lAirportRequestEnabled =  0 ;
			}}
			break;

			case E_UNKNOWN_MODE:
			default:{_ATC_BLOC(1,14);
				 
			break;}
		}

		 
		lAirportRequestActivationEvents =
		...
		{_ATC_LOOP_bef(1,16);for( lI = 0; lI < lCount; lI++ )
		{_ATC_LOOP_in(1,16);{_ATC_BLOC(1,15);{
			lAirportRequestActivationEvent = (AirportRequestActivationEvent*)(lAirportRequestActivationEvents->get(lAirportRequestActivationEvents, lI));
