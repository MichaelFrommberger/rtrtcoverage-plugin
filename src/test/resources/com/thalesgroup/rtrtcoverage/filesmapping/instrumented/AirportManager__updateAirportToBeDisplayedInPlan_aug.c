...
_ATC_DECLARE_BLOC(1,77)
_ATC_DECLARE(1,0x4A147E6CUL,0x7F0D3796UL,2,_ATC_TAB_PROC(1),0,0,77,_ATC_TAB_BLOC(1),0,0)

#line 1 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
...
#line 17 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
...
#line 23 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
#line 24 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
#line 25 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
...
#line 26 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
...
#line 27 "R:\\DGWP-SW-A350\\DGWP_A350_UT\\DGWP_ANF\\Class_AirportManager\\Sources\\AirportManager__updateAirportToBeDisplayedInPlan.c"
...
void AirportManager__updateAirportToBeDisplayedInPlan(AirportManager *pThis) 
{_ATC_LINK(1);{_ATC_PROC(1,1);{_ATC_BLOC(1,76);{
	...
	MapObjectServices *lOriginAirportObject =  ((T_ptr)0) ;
	MapObjectServices *lDestinationAirportObject =  ((T_ptr)0) ;
	T_float32 lOriginAirportLatitude = 0.0f;
	T_float32 lOriginAirportLongitude = 0.0f;
	T_float32 lDestinationAirportLatitude = 0.0f;
	T_float32 lDestinationAirportLongitude = 0.0f;
	T_float32 lDistanceOriginToDestination = 0.0f;
	T_float32 lDistanceAircraftToOrigin = 0.0f;
	T_float32 lDistanceAircraftToDestination = 0.0f;
	...
	if (pThis->_aircraftLocationValidity ==  0 )
	{_ATC_BLOC(1,0);{
		pThis->_defaultAirport =  -1 ;
	}}
	 
	else
	{_ATC_BLOC(1,31);{
		 
		if (pThis->_originAirport ==  -1 )
		{_ATC_BLOC(1,3);{
			 
			if (pThis->_onGroundCondition ==  1 )
			{_ATC_BLOC(1,1);{
				pThis->_defaultAirport = pThis->_nearestAirport;
			}}
			 
			else
			{_ATC_BLOC(1,2);{
				pThis->_defaultAirport = pThis->_destinationAirport;
			}}
		}}
		...
		{_ATC_BLOC(1,30);{
			...
			if (pThis->_destinationAirport ==  -1 )
			{_ATC_BLOC(1,8);{
				...
				{_ATC_BLOC(1,7);{
					...
					if (pThis->_flightPhase < E_FLT_CRUISE)
					{_ATC_BLOC(1,5);{
						pThis->_defaultAirport = pThis->_originAirport;
					}}
			...
			{_ATC_BLOC(1,29);{
				...
				lOriginAirportObject = lDatabaseServices->getAirportInstance(lDatabaseServices, pThis->_originAirport);
				lOriginAirportLatitude = lMapObjectServices->getFloatData(lMapObjectServices, lOriginAirportObject,  3 );
				lOriginAirportLongitude = lMapObjectServices->getFloatData(lMapObjectServices, lOriginAirportObject,  4 );

				lDestinationAirportObject = lDatabaseServices->getAirportInstance(lDatabaseServices, pThis->_destinationAirport);
				lDestinationAirportLatitude = lMapObjectServices->getFloatData(lMapObjectServices, lDestinationAirportObject,  3 );
				lDestinationAirportLongitude = lMapObjectServices->getFloatData(lMapObjectServices, lDestinationAirportObject,  4 );

				lDistanceOriginToDestination = CMathematicalTools->computeOrthodromicDistance(lOriginAirportLatitude, lOriginAirportLongitude,
																							lDestinationAirportLatitude, lDestinationAirportLongitude);

				 
				lDistanceAircraftToOrigin = CMathematicalTools->computeOrthodromicDistance(pThis->_aircraftLatitude, pThis->_aircraftLongitude,
																						lOriginAirportLatitude, lOriginAirportLongitude);
				 
				lDistanceAircraftToDestination = CMathematicalTools->computeOrthodromicDistance(pThis->_aircraftLatitude, pThis->_aircraftLongitude,
																							lDestinationAirportLatitude, lDestinationAirportLongitude);

				 
				if (lDistanceAircraftToOrigin <  50.0f )
				{_ATC_BLOC(1,9);{
					pThis->_transitionPerformed =  0 ;
				}}
				else
				{_ATC_BLOC(1,12);{
					if (pThis->_transitionPerformed ==  0 )
					{_ATC_BLOC(1,10);{
						pThis->_transitionPerformed =  1 ;
						pThis->_firstEntryPlanModeAfterTransition =  1 ;
					}}
					else
					{_ATC_BLOC(1,11);{
				...
				if (lDistanceOriginToDestination <=  300.0f )
				{_ATC_BLOC(1,21);{
					if (pThis->_flightPhaseValidity ==  1 )
					{_ATC_BLOC(1,17);{
						 
						if (pThis->_flightPhase < E_FLT_CRUISE)
						{_ATC_BLOC(1,15);{
							if (lDistanceAircraftToOrigin < lDistanceAircraftToDestination)
							{_ATC_BLOC(1,13);{
								pThis->_defaultAirport = pThis->_originAirport;
							}}
							else
				...
				else {_ATC_BLOC(1,28);if ((lDistanceOriginToDestination >  300.0f ) && (lDistanceOriginToDestination <=  350.0f ))
				{_ATC_BLOC(1,24);{
					if (lDistanceAircraftToDestination <=  300.0f )
					{_ATC_BLOC(1,22);{
						pThis->_defaultAirport = pThis->_destinationAirport;
					}}
					else
	...
	{_ATC_BLOC(1,42);{
		...
		if (pThis->_memorizedAirportInArcNav !=  -1 )
		{_ATC_BLOC(1,32);{
			pThis->_airportToBeDisplayedInPlan = pThis->_memorizedAirportInArcNav;
		}}
		 
		else
		{_ATC_BLOC(1,41);{
			 
			if (pThis->_lastDisplayedAirportInPlan !=  -1 )
			{_ATC_BLOC(1,33);{
				pThis->_airportToBeDisplayedInPlan = pThis->_lastDisplayedAirportInPlan;
			}}
			...
			{_ATC_BLOC(1,40);{
				...
				{_ATC_BLOC(1,36);{
					...
					if (pThis->_destinationAirportOfLastFlight !=  -1 )
					{_ATC_BLOC(1,34);{
						pThis->_airportToBeDisplayedInPlan = pThis->_destinationAirportOfLastFlight;
					}}
					 
					else
					{_ATC_BLOC(1,35);{
						pThis->_airportToBeDisplayedInPlan = pThis->_defaultAirport;
					}}
				}}
				...
				{_ATC_BLOC(1,39);{
					...
					if (pThis->_destinationAirport !=  -1 )
					{_ATC_BLOC(1,37);{
						pThis->_airportToBeDisplayedInPlan = pThis->_destinationAirport;
					}}
	...
	{_ATC_BLOC(1,75);{
		...
		if ((pThis->_originAirport ==  -1 ) || (pThis->_destinationAirport ==  -1 ))
		{_ATC_BLOC(1,51);{
		...
		{_ATC_BLOC(1,74);{
			...
			if ( pThis->_firstDisplayModePlanAfterTransition  ==  1  )
			{_ATC_BLOC(1,56);{
				if( pThis->_memorizedAirportInArcNav !=  -1  )
				{_ATC_BLOC(1,52);{
					pThis->_airportToBeDisplayedInPlan = pThis->_memorizedAirportInArcNav;
				...
				{_ATC_BLOC(1,55);{
					if ((pThis->_airportDisplayedInPlanAfterTransition ==  0 ) ||
						(pThis->_lastDisplayedAirportInPlan ==  -1 ))
					{_ATC_BLOC(1,53);{
						pThis->_airportToBeDisplayedInPlan = pThis->_defaultAirport;
					}}
					else
